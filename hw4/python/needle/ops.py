...
class PowerScalar(TensorOp):
    ...
    def compute(self, a: NDArray) -> NDArray:
        # print("in powerscaler compute")
        # print(a.shape)
        return a ** self.scalar

    def gradient(self, out_grad, node):
        # print("in powerscalar gradient")
        # print(node.inputs[0].shape, out_grad.shape)
        # print(node.inputs[0].device, out_grad.device)
...
class Conv(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        # out_grad: N * (H+2P-K+1) * (W+2P-K+1) * C_out, P = self.padding
        X_grad = conv(out_grad, W_permute, padding=K-1-self.padding) # N * H * W * C_in

        X_permute = transpose(X, (0, 3)) # C_in * H * W * N
        grad_permute = transpose(transpose(out_grad, (0, 1)), (1, 2)) # (H+2P-K+1) * (W+2P-K+1) * N * C_out
        W_grad = conv(X_permute, grad_permute, padding=self.padding) # W_grad: C_in * (H+2P-(H+2P-K+1)+1) * (W+2P-(W+2P-K+1)+1) * C_out
        W_grad = transpose(transpose(W_grad, (0, 1)), (1, 2)) # (H+2P-K+1) * (W+2P-K+1) * C_in * C_out
