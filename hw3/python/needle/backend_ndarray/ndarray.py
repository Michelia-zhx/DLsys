...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        assert prod(self._shape) == self._handle.size, "product of current shape is not equal to the product of the new shape"
        assert self.is_compact(), "The original array is not compact"
    ...
    def broadcast_to(self, new_shape):
        ...
        assert len(self._shape) == len(new_shape), \
            "operands could not be broadcast together with shape {} and requested shape {}".format(self._shape, new_shape)
        for i in range(len(self._shape)):
            assert self._shape[i] != 1 or new_shape[i] != self._shape[i], \
                "operands could not be broadcast together with shape {} and requested shape {}".format(self._shape, new_shape)
        strides = tuple([self.strides[i] if self._shape[i] != 1 else 0 for i in range(len(self._shape))])
        ret = NDArray.make(
    ...
    def __getitem__(self, idxs):
        ...
        shape = tuple([(sl.stop - sl.start + sl.step - 1) // sl.step for sl in idxs])
        offset = sum([self.strides[i] * idxs[i].start for i in range(len(idxs))])
